// orders.js — wires orders.html to /order/* endpoints
// orders.html doesn't ship any modal markup, so this file injects a
// minimal one at runtime for creating orders and viewing details.
...
async function confirmDelivery(orderId) {
  ...
  try {
    const res = await fetch(`/order/confirm_delivery/${orderId}`, { method: 'POST' });
    const data = await res.json();
...
function injectModal() {
  ...
  modal.querySelector('#createOrderForm').addEventListener('submit', async (e) => {
    ...
    try {
      const res = await fetch('/order/create_order/', {
        method: 'POST',
